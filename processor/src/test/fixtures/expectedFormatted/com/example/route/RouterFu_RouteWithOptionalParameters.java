...
public class RouterFu_RouteWithOptionalParameters extends RouteWithOptionalParameters
    implements RouteWithOptionalParametersService {
  ...
  private final Route $fu$_route_regionEvents = new Route(
      ...
      new Segment[] {
        new Segment("/regions/"),
        new Segment($fu$_regionCode),
        new Segment("/events/"),
        new Segment($fu$_filter),
      },
      new Parameter[] {
        $fu$_regionCode, $fu$_filter,
      },
      new RegExp(
  ...
  private final Route $fu$_route_regionEvent = new Route(
      ...
      new Segment[] {
        ...
        new Segment($fu$_regionCode),
        new Segment("/event/"),
        new Segment($fu$_eventId),
        new Segment("/"),
        new Segment($fu$_filter),
      },
      new Parameter[] {
        $fu$_regionCode, $fu$_eventId, $fu$_filter,
      },
      new RegExp(
          "^/regions/((?:[a-zA-Z0-9\\-_\\$\\.\\+!\\*'\\(\\)\\,\\\"~]|%[A-F0-9]{2})*)/event/((?:[a-zA-Z0-9\\-_\\$\\.\\+!\\*'\\(\\)\\,\\\"~]|%[A-F0-9]{2})+)/((?:[a-zA-Z0-9\\-_\\$\\.\\+!\\*'\\(\\)\\,\\\"~]|%[A-F0-9]{2})*)$"),
      (location, route, parameters) -> MatchResult.TERMINAL);
  ...
  @Nonnull
  @Override
  public String buildRegionEventsLocation(
      @Nonnull final String regionCode, @Nonnull final String filter) {
    final Map<Parameter, String> $fu$_route_params = new HashMap<>();
    $fu$_route_params.put($fu$_regionCode, regionCode);
