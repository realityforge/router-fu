...
public class RouterFu_RouteParametersWithShortName extends RouteParametersWithShortName
    implements RouteParametersWithShortNameService {
  private final Parameter $fu$_regionCode = new Parameter("regionCode");

  private final Route $fu$_route_A = new Route(
      "A",
      new Segment[] {
  ...
  RouterFu_RouteParametersWithShortName(@Nonnull final Backend backend) {
    $fu$_router = new Router(
        ...
        backend,
        Collections.unmodifiableList(Arrays.asList($fu$_route_A)));
    $fu$_router.activate();
  }
